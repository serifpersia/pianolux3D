...
@export var black_note_mesh_scene: PackedScene
...
func _ready() -> void:
	...
	var key_mesh_instance = black_note_mesh_scene.instantiate()
...
func on_note_on(pitch: int) -> void:
	if not midi_keyboard.is_black_key(pitch):
		return
	var key_mesh_instance: Node3D = black_note_mesh_scene.instantiate()
