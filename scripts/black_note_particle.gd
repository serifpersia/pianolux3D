...
@export var note_on_black_key_particles_material: StandardMaterial3D
...
func spawn_particle(pitch: int) -> void:
	...
	for child_node in particle_instance.get_children():
		if child_node is GPUParticles3D or child_node is CPUParticles3D or child_node is MeshInstance3D:
			if note_on_black_key_particles_material:
				var new_child_material = note_on_black_key_particles_material.duplicate(true) as StandardMaterial3D
				if new_child_material:
					new_child_material.albedo_color = midi_keyboard.black_note_mesh_color
